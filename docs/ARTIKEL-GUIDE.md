...
4. **Admin (10. sep. 2026).** Alle tre sider styres fra `/admin/`, delt i tre rum som sitet: artikler (`manual.json`), politiske skandaler (politikere/manifest/skandaler/løfter) og Skattejægeren (én JSON pr. sag). Gem kladde. Udgiv aldrig før Matt trykker Udgiv. Se `docs/ADMIN.md`.
...
Artikler lever i **`data/export.json`** under `articles[]`. Nye og rettede artikler fra admin ligger i **`data/manual.json`** og overskriver samme slug ved build. Kladder (`status: "draft"`) kommer ikke på sitet.
